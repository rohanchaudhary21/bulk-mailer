...
def send_bulk(user_email, recipients, subject, body, delay, sheet_data=None, attachments=None):
    ...
    for i, email in enumerate(recipients):
        try:
            # Replace template variables with data from sheet
            personalized_subject = subject
            personalized_body = body
            
            if sheet_data and i < len(sheet_data):
                row_data = sheet_data[i]
                # Replace all {{ column_name }} with actual values
...
def send():
    ...
    if send_type == "now":
        ...
        thread = threading.Thread(
            target=send_bulk,
            args=(session["user_email"], recipients, subject, body, delay, sheet_data)
        )
...
def debug_database():
    ...
    # Get OAuth tokens (without showing full token)
    cursor.execute("SELECT user_email FROM oauth_tokens")
    tokens = cursor.fetchall()
    
    # Get email logs
    cursor.execute("SELECT email, status, created_at FROM email_logs ORDER BY created_at DESC LIMIT 20")
    ...
    return {
        ...
        "authenticated_users": [dict(t) if DATABASE_URL else {"user_email": t[0]} for t in tokens],
        "total_emails": total,
        "sent_emails": sent,
